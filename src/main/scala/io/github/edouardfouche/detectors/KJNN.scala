...
// In this version, scores are not weighted by the similarity. (i..e, not as in the paper)
case class KJNN(rstarwords: ElkiKNN, rstardocs: ElkiKNN, doclabels: Map[Vector[Double], Int], wordlabels: Map[Vector[Double], String],
        ...
        for{
          ...
          val score = if(j == 0) {
            ...
            val relevance = nn_docwords.map(x => {
              ...
              repscores(c).getOrElse(wordlabels(x.toVector),0.0)
            }).sum
            //similarity *
            relevance
          }
