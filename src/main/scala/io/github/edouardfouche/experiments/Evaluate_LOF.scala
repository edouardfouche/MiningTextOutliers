...
/**
  *
  */
object Evaluate_LOF   extends Experiment {
  def run(): Unit = {
    ...
    for{
      ...
      val ks: Vector[Int] = dynamicrange(1, maxk, 0.30)

      for {
        k <- ks.par
      } {
        val (cpu, wall, scores: Array[(Int, Double)]) = StopWatch.measureTime(ElkiLOF(k).computeScores(alldocs))
        ...
        val attributes = List("scoreId", "dataset", "bmId", "k", "ndocs", "emb", "cpu", "wall",
